...
typealias RawAttributes = [String: Any]
...
extension Attributes {
    ...
    init(rawAttributes attributes: RawAttributes) {
        ...
        if let paragraph = attributes[NSParagraphStyleAttributeName] as? NSParagraphStyle {
            self.alignment = paragraphStyleCompare(paragraph) { $0.alignment }
            self.leading = paragraphStyleCompare(paragraph) { Float($0.lineSpacing) }
            self.lineHeightMultiplier = paragraphStyleCompare(paragraph) { Float($0.lineHeightMultiple) }
            self.paragraphSpacingAfter = paragraphStyleCompare(paragraph) { Float($0.paragraphSpacing) }
            self.paragraphSpacingBefore = paragraphStyleCompare(paragraph) { Float($0.paragraphSpacingBefore) }
            self.headIndent = paragraphStyleCompare(paragraph) { Float($0.headIndent) }
            self.tailIndent = paragraphStyleCompare(paragraph) { Float($0.tailIndent) }
            self.firstLineHeadIndent = paragraphStyleCompare(paragraph) { Float($0.firstLineHeadIndent) }
            self.minimumLineHeight = paragraphStyleCompare(paragraph) { Float($0.minimumLineHeight) }
            self.maximumLineHeight = paragraphStyleCompare(paragraph) { Float($0.maximumLineHeight) }
            self.hyphenationFactor = paragraphStyleCompare(paragraph) { Float($0.hyphenationFactor) }
            if #available(iOS 9.0, *) {
                self.allowsTighteningForTruncation = paragraphStyleCompare(paragraph) { $0.allowsDefaultTighteningForTruncation }
            }
        }
    ...
    private func paragraphStyleCompare<U: Equatable>(_ paragraph: NSParagraphStyle, trans: (NSParagraphStyle) -> U) -> U? {
...
extension Attributes {
    ...
    private mutating func setTrait(_ trait: UIFontDescriptorSymbolicTraits, enabled: Bool) {
...
extension Attributes {
    ...
    private func isAnyNotNil(_ objects: Any? ...) -> Bool {
...
extension NSAttributedString {
    var runningAttributes: RawAttributes? {
        guard length > 0 else {
            return nil
        }
...
public extension NSMutableAttributedString {
    public typealias AttributeSetter = (inout Attributes) -> Void
